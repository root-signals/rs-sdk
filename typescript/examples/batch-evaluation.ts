  ...
  try {
    ...
    const evaluationData: EvaluationItem[] = [
      ...
      },
      {
        ...
        response:
          'You can reach our customer support team via email at support@company.com or call us at 1-800-SUPPORT.',
      },
      {
        id: 'eval-4',
      ...
      },
      {
        ...
        response:
          'The user manual is available in the Downloads section of your account dashboard.',
      },
    ];
    ...
    for (let i = 0; i < evaluationData.length; i++) {
      ...
      console.log(
        `    Score: ${result.score?.toFixed(3) || 'No score'} - ${item.request.substring(0, 30)}...`,
      );
    }
    ...
    for (let i = 0; i < evaluationData.length; i += batchSize) {
      ...
      console.log(
        `  Processing batch ${Math.floor(i / batchSize) + 1}: items ${i + 1}-${Math.min(i + batchSize, evaluationData.length)}`,
      );
      ...
      batchResults.forEach(({ item, result }) => {
        console.log(
          `    ${item.id}: Score ${result.score?.toFixed(3) || 'No score'} - ${item.request.substring(0, 30)}...`,
        );
      });
    }
    ...
    multiEvalResults.forEach(({ evaluator, result, error }) => {
      if (error) {
        ...
        console.log(`    ${evaluator}: Score ${result.score?.toFixed(3) || 'No score'}`);
      }
    });
    console.log();
