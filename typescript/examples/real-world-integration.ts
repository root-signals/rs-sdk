...
class ProductionEvaluationService {
    ...
    try {
      ...
      const evaluationPromises = evaluators.map(async (evaluatorName) => {
        try {
          ...
          console.log(`  ✅ [${evaluationId}] ${evaluatorName}: ${result.score.toFixed(3)}`);
          
        } catch (error) {
          console.error(`  ❌ [${evaluationId}] Failed to evaluate ${evaluatorName}:`, 
      ...
      const validScores = Object.values(scores).map(s => s.score).filter(s => s !== undefined);
      ...
      console.log(`📊 [${evaluationId}] Overall score: ${overallScore.toFixed(3)}, Issues: ${issues.length}`);
  ...
  async batchEvaluate(responses: ChatbotResponse[]): Promise<void> {
    ...
    for (let i = 0; i < responses.length; i += batchSize) {
      ...
      console.log(`📈 Progress: ${progress}/${responses.length} (${(progress/responses.length*100).toFixed(1)}%)`);
    }
    
    console.log(`✅ Batch evaluation completed. ${results.length}/${responses.length} successful evaluations`);
  }
  ...
  async abTestResponses(
    ...
    const variantScore = variantScores.reduce((sum, score) => sum + score, 0) / variantScores.length;
  ...
  private updateMetrics(score: number, model: string, issues: string[]): void {
    ...
    if (!this.metrics.modelPerformance[model]) {
      ...
      const modelEvals = Object.values(this.metrics.scoreDistribution).reduce((sum, count) => sum + count, 0);
      this.metrics.modelPerformance[model] = (
        (this.metrics.modelPerformance[model] * (modelEvals - 1)) + score
      ) / modelEvals;
    }
...
async function main() {
  try {
    ...
    const sampleResponses: ChatbotResponse[] = [
      {
        ...
        response: 'You can cancel your subscription anytime from your account settings under the billing section.',
        timestamp: new Date(),
        model: 'gpt-4',
        responseTime: 1200
      },
    ...
    console.log(`   Result: ${singleResult.overallScore.toFixed(3)} overall score, ${singleResult.issues.length} issues\n`);
    ...
    const variantResponses = [{
      ...sampleResponses[0],
      id: 'resp_variant',
      response: 'I\'d be happy to help you cancel your subscription! You can do this instantly from your account settings under billing, or I can walk you through the process step by step. Would you prefer me to guide you through it?'
    }];

    await service.abTestResponses('Subscription Cancellation Response', baselineResponses, variantResponses);
    console.log();
