...
async function main() {
  try {
    ...
    if (aiJudgeResult.error_code) {
      ...
      console.log(`  📋 Intent: ${aiJudge.intent}`);
      console.log(`  📋 Evaluators: ${aiJudge.evaluator_references?.length || 0} evaluators`);
      console.log();
      ...
      const aiJudgeResult1 = await client.judges.execute(aiJudgeResult.judge_id, {
        ...
        response: "I completely understand your frustration, and I sincerely apologize for the delay. Let me immediately look into your order status and provide you with a tracking update. I'll also arrange for expedited shipping on your replacement order at no extra cost."
      });

      console.log(`  📊 Evaluation Results:`);
      if (Array.isArray(aiJudgeResult1.results)) {
        aiJudgeResult1.results.forEach((result: any, index: number) => {
          console.log(`    ${index + 1}. ${result.evaluator_name || 'Evaluator'}: Score ${result.score?.toFixed(3) || 'N/A'}`);
    ...
    const evaluators = await client.evaluators.list({ limit: 3 });
    if (evaluators.results.length >= 2) {
      const evaluatorRefs = evaluators.results.slice(0, 2).map(eval => ({ id: eval.id }));
      
      const customJudge = await client.judges.create({
        name: "E-commerce Support Judge",
        intent: "Evaluate e-commerce customer support responses for accuracy, helpfulness, and policy compliance",
        evaluator_references: evaluatorRefs
      });

      console.log(`  ✅ Created custom judge: ${customJudge.name}`);
      console.log(`  📋 ID: ${customJudge.id}`);
      console.log(`  📋 Evaluators: ${customJudge.evaluator_references?.length || 0}`);
      console.log();
      ...
      const testScenarios = [
        {
          ...
          response: "We offer a 30-day return policy for electronics. Items must be in original packaging and condition. You can initiate a return through your account or contact our support team."
        },
        {
          name: "Shipping Delay Complaint", 
          request: "My package was supposed to arrive yesterday but it's still not here.",
          response: "I apologize for the shipping delay. Let me track your package immediately and provide you with an updated delivery estimate. If there are further delays, I'll arrange priority shipping for your next order."
        }
      ];
      ...
      for (const scenario of testScenarios) {
        ...
        if (Array.isArray(result.results)) {
          result.results.forEach((evalResult: any, index: number) => {
    ...
    if (!aiJudgeResult.error_code) {
      ...
      const refinementResult = await client.judges.refine(aiJudgeResult.judge_id, refinementPayload);
    ...
    const responseTypes = [
      {
        type: "Professional",
        response: "Thank you for contacting us. I understand your concern and will investigate this matter immediately. I'll provide you with a detailed update within 24 hours."
      },
      {
        type: "Casual", 
        response: "Hey! Yeah, that's definitely weird. Let me check what's going on and get back to you soon!"
      },
      {
        type: "Minimal",
