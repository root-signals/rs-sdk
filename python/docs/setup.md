...
1. as an argument to Scorable constructor (see below),
2. environment variable `SCORABLE_API_KEY` (see quickstart), or
3. .env file containing `SCORABLE_API_KEY=` (see below)

### Provide it in the code (highest priority)

In addition to the `SCORABLE_API_KEY` environment variable, you can also supply the API key directly from your code using either:

```
from scorable import Scorable
client = Scorable("my secret api key")
...
```
...
```
from scorable import Scorable
client = Scorable(api_key="my secret api key")
...
```

### .env file (lowest priority)

Current directory where the Python interpreter is being executed can be also used to find the `.env` file with `variable=value` definitions. In this case, if there is `SCORABLE_API_KEY=mysecretapikey` entry there, it is used.
